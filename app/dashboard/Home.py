...
if prompt := st.chat_input("Ρωτήστε για δημόσιες δαπάνες... (π.χ. Ποιο υπουργείο έχει τις περισσότερες επισημάνσεις;)"):
    ...
    with st.chat_message("assistant"):
        ...
        logger.info(f"Query: '{prompt}' → filters={filters}, semantic='{semantic_query}'")
        ...
        try:
            ...
            relevant_contracts = vs.search_contracts(prompt, n_results=20)

        logger.info(f"Retrieved {len(relevant_contracts)} contracts before reranking.")
        ...
        if not relevant_contracts:
            ...
            # 3. Re-rank
            relevant_contracts = vs.rerank_results(relevant_contracts, prompt, top_k=15)

            orgs = set(c.get("organization", "") for c in relevant_contracts)
            risks = [c.get("risk_level", "") for c in relevant_contracts]
            logger.info(f"After reranking: {len(relevant_contracts)} contracts, orgs={orgs}, risks={risks}")
